...
let
  ...
  transformMcpForOpencode = name: server:
    let
      baseServer = if server ? enable
        then (builtins.removeAttrs server ["enable"]) // { enabled = server.enable; }
        else server;
    in
    if baseServer.type == "http" then
      ...
      let
        withoutOldFields = builtins.removeAttrs baseServer ["type" "command" "args" "env"];
      ...
      in
      if baseServer ? env
        then withCommand // { environment = baseServer.env; }
        else withCommand
    else
      baseServer;
  ...
  generateOpencodeFrontmatter = name: agent:
    let
      ...
      ++ lib.optional (agent.opencodeModel != null) "model: ${agent.opencodeModel}"
      ++ lib.optional (agent.temperature != null) "temperature: ${toString agent.temperature}"
  ...
  generateAgentFile = format: name: agent:
    let
      frontmatter = if format == "claude"
        then generateClaudeFrontmatter name agent
        else generateOpencodeFrontmatter name agent;
...
'';
    in
    if format == "claude"
      then pkgs.writeText "${name}.md" fullContent
      else fullContent;  # opencode expects text, not derivation

  # Convert agents to directories for each tool
  claudeAgentsDir = if cfg.agents != {} then
    pkgs.linkFarm "claude-agents" (lib.mapAttrsToList
      (name: agent: {
        name = "${name}.md";
  ...
  opencodeAgents = lib.mapAttrs
    (name: agent: generateAgentFile "opencode" name agent)
    (lib.filterAttrs (name: agent: !agent.disable) cfg.agents);
  ...
  agentType = types.submodule {
    options = {
      content = mkOption {
        type = types.path;
        description = "Path to agent content markdown file (without frontmatter)";
      };
      ...
      tools = mkOption {
        ...
        default = null;
        description = "List of tools for claude-code (Glob, Grep, Read, Write, etc.)";
      };
      ...
      opencodeModel = mkOption {
        type = types.nullOr types.str;
        default = null;
        description = "Full model identifier for opencode (e.g., anthropic/claude-sonnet-4)";
      };
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    programs.claude-code = mkIf cfg.enableClaudeCode (mkMerge [
      ...
      (mkIf (cfg.memory != null) {
        memory.source = cfg.memory;
      })
    ]);
